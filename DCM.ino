...
void BuildDCM(Matrix3* Mat, float someRoll, float somePitch, float someHeadingAngle) {
    ...
    Mat->Y = Vector3(cosPitch * sinHeading,  
    ...
    /* Mat->X = Vector3(cosPitch * cosHeading,  
...
void Update(boolean rate) {
    ...
    vEstAccel = Vector3(-sinTheta * sin(headingAngle), 
                        sinTheta * cos(headingAngle), 
                        -cosTheta);
    ...
    pitchRate = pitchRate * BETA + (vGyro.y * cos(roll) + 
                                    vGyro.z * sin(roll)) * (1-BETA);
...
void CorrectHeading(void) {
    ...
    Rk = Matrix2((SIGMA_angle * SIGMA_angle) / cos(pitch), 
                0, 0, vGyroVariance.z);
