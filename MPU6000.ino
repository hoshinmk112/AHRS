...
volatile int _MPU6000_newdata = 0;
...
// FIFO variables
static uint8_t _received_packet[72];
static uint8_t _fifoCountH;
static uint8_t _fifoCountL;

static long _accel_filtered[3];
static int  _accel_filtered_samples;
static float _gyro_bias[3]; // bias_tracking
static float _gyro_bias_from_gravity_gain;  // bias correction algorithm gain
static uint8_t _gyro_bias_from_gravity_counter;
// internal DCM Rotation matrix (in internal chip axis definition)
static float _DCM[3][3];

// Compass correction variables
static float _yaw_compass_diff;
static float _compass_correction_gain;
static float _gyro_bias_from_compass_gain;  // default value (not used now)
...
static unsigned char dmpMem[8][16][16] = {
   ...
   ,{{0xD9, 0xD8, 0xD8, 0xF1, 0x9E, 0x88, 0xA3, 0x31, 0xDA, 0xD8, 0xD8, 0x91, 0x2D, 0xD9, 0x28, 0xD8}
    ...
    ,{0xB9, 0xA7, 0xF1, 0x26, 0x26, 0x26, 0xD8, 0xD8, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
    ,{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
    ,{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
    ,{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
    ,{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
...
void MPU6000_init(void) {
    ...
    SPI.setClockDivider(SPI_CLOCK_DIV16);      // SPI at 16Mhz (on 16Mhz clock)
    delay(10);
    ...
    // FS & DLPF   FS=2000‚à´/s, DLPF = 42Hz (low pass filter)

    _SPI_write(MPUREG_CONFIG, BITS_DLPF_CFG_42HZ);  // BITS_DLPF_CFG_20HZ BITS_DLPF_CFG_42HZ BITS_DLPF_CFG_98HZ
    _SPI_write(MPUREG_GYRO_CONFIG, BITS_FS_500DPS);  // Gyro scale 500‚à´/s
    _SPI_write(MPUREG_ACCEL_CONFIG, BITS_FS_4G);           // Accel scele 4g (g=2*8192)
...
void MPU6000_set_dmp_rate(uint8_t rate) {
    _set_fifo_rate(rate);
}
...
void MPU6000_calculate(void) {
   ...
   */
    ...
    // First Rotate 90‚à´ over Z axis
    // Second Rotate 180‚à´ over X axis
    // R = [0 1 0, 1 0 0, 0 0 -1]
    // DCM = R*_DCM*Rt
...
void MPU6000_gyro_offset_calibration(void) {
...
void MPU6000_accel_set_offset(int accel_x_offset, int accel_y_offset, int accel_z_offset) {
    // long aux_long;
    int aux_int;
    uint8_t regs[2];
...
void MPU6000_accel_get_offset(int *accel_offset) {
...
void MPU6000_accel_offset_calibration(void) {
    float aux_float[3] = {0, 0, ACCEL_SCALE_G}; // Initial expected values...
    int i;
    // int j;
    // int aux_int;
    // uint8_t aux_byte;
    ...
    MPU6000_accel_set_offset(MPU6000_ACCEL_OFFSET[0], MPU6000_ACCEL_OFFSET[1], MPU6000_ACCEL_OFFSET[2]);
}
...
float MPU6000_compass_angle(float mag_x, float mag_y, float mag_z, float declination) {
    float headX;
    float headY;
    float heading;
...
void MPU6000_update_yaw_compass_correction(float compass_heading) {
    // float corrected_yaw;
    ...
    // Update the differential component to redude the error (it¬•s like a P control)
...
void MPU6000_get_yaw_compass_corrected(void) {
    // safety check
...
void MPU6000_gyro_bias_correction_from_gravity(void) {
...
void MPU6000_set_accel_fusion_gain(uint8_t gain) {
    // safety checks
    if (gain < 0x20) {
        gain = 0x20;
    }
...
void MPU6000_set_gyro_bias_correction_from_gravity_gain(float gain) {
    _gyro_bias_from_gravity_gain = gain;
}

// Funtion to set the gain of the compass fusion
void MPU6000_set_compass_correction_gain(float gain) {
...
static void _gyro_offset_update(int offsetX, int offsetY, int offsetZ) {
    int aux_int;
